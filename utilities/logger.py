...
AGENT_TRAIN_FORMAT = {
    ...
    "hier": [
        ("batch_reward", "BR", "float"),
        ("action_loss", "ALOSS", "float"),
        ("value_loss", "VLOSS", "float"),
        ("actor_entropy", "AENT", "float"),
    ],
}

KEYS = tuple(x[0] for x in AGENT_TRAIN_FORMAT["inc"])
KEYS += tuple(x[0] for x in COMMON_TRAIN_FORMAT)
...
class Logger(object):
    def __init__(self, log_dir, save_tb=False, log_frequency=10000, agent="inc"):
    ...
    def log_param(self, key, param, step, log_frequency=None):
        # Disable logging parameters for now
        return
